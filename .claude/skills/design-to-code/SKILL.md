...
`CLAUDE.md` is authoritative on *what* each rule is; this file on *when and how* to apply it.
...
| Effect impossible from outside | **Only then** add an *optional* prop with a behaviour-preserving default, and note it in the header comment |
...
1. **Container** — `<main>` colour; stage `max-w-*` per breakpoint; height model.
2. **Layout skeleton** — the flex/grid tree top to bottom, display mode named per node.
   *Every* intended `absolute` needs a one-line justification here.
...
changes above them: *"the mobile frame places it at y382 … so each Figma y is the original
minus that"* (`section-27/neural-diagram.tsx`).
...
**Checkpoint:** does the skeleton hold at 402, 600, 744, 1000, 1280, 1440, 1920?
...
- Prefer sizing a wash off the *stage*, not a fixed px, whenever it must reach the edges.
...
> JS.** `section-28/dot-halo.tsx`: *"Both are canvas geometry, not CSS, so they switch on a
...
5. Report done. State explicitly anything that could not be matched, and why.
