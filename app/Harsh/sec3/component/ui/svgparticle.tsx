                    ...
                    for (let x = 0; x < W; x += gap) {
                        ...
                        if (px[i + 3] >= 20)
                            src.push({
                                ...
                                g: px[i + 1],
                                b: px[i + 2],
                                // In single-colour mode the sampled alpha is
                                // just the artwork's anti-aliasing, which makes
                                // edge dots render darker than the rest. The
                                // colour is uniform, so the opacity should be
                                // too.
                                a:
...
export default function ParticleImage(props: Record<string, unknown>) {
  return <__OriginkitBase_ParticleImage {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
