...
export default function StickerPeeling(__props: {
  ...
  const recreateMeshForSize = useCallback(
    (meshW: number, meshH: number) => {
      ...
      if (loadedImageRef.current) {
        ...
        const backColorRgba2 = parseColorToRgba(resolveTokenColor(backColor));
        let rawBackTexture = preservedTextures.back;
        if (!rawBackTexture) {
          rawBackTexture =
            backColorRgba2.a <= 0 ? texture : createBackTexture(img, backColor);
        }
    ...
    for (let i = 0; i < bones.length; i++) {
      ...
      if (signedDist > 0) {
        ...
        if (curlMode === "semicircle") {
          ...
          const effectiveR = radiusWorld * Math.pow(0.85, angle_sp / Math.PI);
  ...
  if (contextLost) {
    return (
      ...
      >
        WebGL context lost — remove some components from the canvas and re-add
        this one.
      </div>
    );
  }
