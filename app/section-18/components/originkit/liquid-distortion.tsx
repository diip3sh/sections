...
export default function LiquidHover({
  ...
  useEffect(() => {
    ...
    // premultipliedAlpha: true (default) — fragment must write rgb*a.
    // Writing premultiplied color into a straight-alpha canvas darkens soft edges
    // into a muddy halo against bright backgrounds (the red spotlight).
    ...
    // 1 = fill the container (no letterboxed dead zone under the bust)
    const innerScale = 1;
    const pointer = {
...
vec4 sample_image_smooth(vec2 uv) {
  vec2 uvc = clamp(uv, 0.0, 1.0);
  vec4 base = texture2D(u_text_texture, vec2(uvc.x, 1.0 - uvc.y));
  ...
  if (outOfBounds > 0.0) {
    float d = 0.002;
    vec4 sum = vec4(0.0);
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x - d, 0.0, 1.0), 1.0 - clamp(uvc.y - d, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x, 0.0, 1.0), 1.0 - clamp(uvc.y - d, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x + d, 0.0, 1.0), 1.0 - clamp(uvc.y - d, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x - d, 0.0, 1.0), 1.0 - clamp(uvc.y, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x, 0.0, 1.0), 1.0 - clamp(uvc.y, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x + d, 0.0, 1.0), 1.0 - clamp(uvc.y, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x - d, 0.0, 1.0), 1.0 - clamp(uvc.y + d, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x, 0.0, 1.0), 1.0 - clamp(uvc.y + d, 0.0, 1.0)));
    sum += texture2D(u_text_texture, vec2(clamp(uvc.x + d, 0.0, 1.0), 1.0 - clamp(uvc.y + d, 0.0, 1.0)));
...
void main () {
  ...
  vec4 tex = sample_image_smooth(img_uv);
  float opacity = get_img_frame_alpha(frame_uv, .002) * tex.a;
  gl_FragColor = vec4(tex.rgb * opacity, opacity);
}
`;
