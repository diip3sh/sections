...
const DEFAULT_IMAGES = [
  "/section-20/portrait/portrait-01.png",
  "/section-20/portrait/portrait-02.png",
  "/section-20/portrait/portrait-03.png",
  "/section-20/portrait/portrait-04.png",
  "/section-20/portrait/portrait-05.png",
  "/section-20/portrait/portrait-06.png",
];
...
function __OriginkitBase_ImageBox(props: Partial<ImageBoxProps>) {
  ...
  const urls = useMemo(() => {
    ...
    const baseList = list.length ? list : DEFAULT_IMAGES;
    // Repeat base list 5 times so 6 images become 30 images in the grid depth
    return Array(5).fill(baseList).flat();
  }, [images]);
  ...
  useEffect(() => {
    ...
    // Adjacency map: for each slot index, the set of neighbour slot indices.
    // Two slabs are adjacent if they share an edge on the tunnel cross-section.
    //
    // SLOTS layout (for cols C, rows R):
    //   floor   col i  → index 2*i
    //   ceiling col i  → index 2*i + 1
    //   left  wall row j → index 2*C + 2*j
    //   right wall row j → index 2*C + 2*j + 1
    const adjacency: Set<number>[] = SLOTS.map(() => new Set<number>());
    ...
    // Same-face neighbours (consecutive slots along each face)
    for (let i = 0; i < cols - 1; i++) {
      link(2 * i, 2 * (i + 1));     // floor:   col i ↔ col i+1
      link(2 * i + 1, 2 * (i + 1) + 1); // ceiling: col i ↔ col i+1
    }
    for (let j = 0; j < rows - 1; j++) {
      link(2 * cols + 2 * j, 2 * cols + 2 * (j + 1));     // left wall:  row j ↔ j+1
      link(2 * cols + 2 * j + 1, 2 * cols + 2 * (j + 1) + 1); // right wall: row j ↔ j+1
    }

    // Corner neighbours (where floor/ceiling meets the walls)
    link(2 * 0, 2 * cols + 2 * 0);            // floor[0]      ↔ left-wall[0]  (bottom-left)
    link(2 * (cols - 1), 2 * cols + 2 * 0 + 1);        // floor[C-1]    ↔ right-wall[0] (bottom-right)
    link(2 * 0 + 1, 2 * cols + 2 * (rows - 1));   // ceiling[0]    ↔ left-wall[R-1](top-left)
    link(2 * (cols - 1) + 1, 2 * cols + 2 * (rows - 1) + 1); // ceiling[C-1] ↔ right-wall[R-1](top-right)
    ...
    function populate(group: THREE.Group) {
      ...
      // Track which slot indices are made visible in this pass so we can
      // block their neighbours from also becoming visible.
      const visibleInPass = new Set<number>();
      ...
      for (let idx = 0; idx < slabs.length; idx++) {
        ...
        // Reject if any adjacent slot is already visible in this segment.
        const blocked = [...adjacency[idx]].some((n) => visibleInPass.has(n));
        if (blocked) {
          slab.visible = false;
          continue;
        }
  ...
  }, [urls, background, lineColor, lineOpacity, grid, fade]);
