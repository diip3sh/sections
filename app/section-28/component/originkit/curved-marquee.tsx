...
export default function CurvedLoop({
  ...
  useEffect(() => {
    if (!ready) return;
    // tspansRef is only ever written to, never trimmed. The first pass runs
    // before the text is measured, so spacing is gap-only and the repeat count
    // comes out an order of magnitude too high; once the real width lands the
    // count collapses but the stale (now detached) entries stay in the array.
    // maxX is derived from its length, so those leftovers stretch the wrap
    // range far past the end of the path — the live copies spend most of the
    // loop beyond it, where they stop rendering, and pile up at the path end
    // on the way out. Trimming to the live count is what keeps it looping.
    tspansRef.current.length = calculatedRepeats;
    let raf = 0;
