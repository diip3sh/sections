...
// ── Easing from a Framer Transition ─────────────────────────────────────────
...
interface PixelCardProps {
  ...
  /** Run the appear + shimmer loop on mount instead of waiting for hover. */
  autoPlay?: boolean;
  transition?: any;
...
export default function PixelCard(props: PixelCardProps) {
  ...
  // Export/thumbnail → one static frame. Canvas → auto-run appear+shimmer so
  // the pixels blink per the selected speed. Preview → wait for hover.
  const present = () => {
    if (isExport) drawStaticFrame();
    else if (isCanvas || autoPlay) handleAnimation("appear");
  };
  ...
  return (
    <div
      ref={containerRef}
      style={rootStyle}
      onMouseEnter={isExport || isCanvas || autoPlay ? undefined : onMouseEnter}
      onMouseLeave={isExport || isCanvas || autoPlay ? undefined : onMouseLeave}
      tabIndex={-1}
    >
      <canvas
        ref={canvasRef}
...
const COMPONENT_DEFAULTS = {
  colors: DEFAULT_COLORS,
  appearFrom: "middle" as AppearFrom,
  gap: 6,
  pixelSize: 2,
  speed: 80,
  padding: 0,
