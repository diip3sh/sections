  ...
  useEffect(() => {
    ...
    const rimMaterial = new ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          float fresnel = 1.0 - max(dot(normalize(vNormal), normalize(vViewDirection)), 0.0);
          fresnel = pow(fresnel, 4.2);

          float topMask = smoothstep(0.05, 0.85, normalize(vWorldNormal).y);
          float opacity = fresnel * mix(0.08, 0.52, topMask);

          gl_FragColor = vec4(vec3(0.9), opacity);
        }
      `,
    });
    const rimMesh = new Mesh(
      new SphereGeometry(globeRadius * 1.003, 64, 64),
      rimMaterial,
    );
    ...
    const loadWorldData = async () => {
      try {
        const landFeatures = await getLandData();
    ...
    const ambientLight = new AmbientLight(0xffffff, 0.035);
    const rimLight = new DirectionalLight(0xffffff, 0.65);
    rimLight.position.set(0, 3.5, -5);
    const fillLight = new DirectionalLight(0xffffff, 0.06);
