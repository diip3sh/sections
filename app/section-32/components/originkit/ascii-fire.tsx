// Ascii Flame — Originkit
// Using component defaults, plus one added prop: `fontSize` (default 10, the
// value that was hard-coded), because the character cell is the one thing the
// component never exposed and section-32 needs it at three different sizes. It
// drives both the glyph and the column/row measurement, so it cannot be faked
// with a CSS transform — a scaled ancestor makes getBoundingClientRect report
// the scaled box and the field lays out at the wrong row count.
...
const seedFuel = (
  ...
  const pulseMultiplier = config.pulse
    ...
    : 1;
  const fuelRows = clamp(
    Math.round(config.thickness),
    1,
    Math.max(1, rows - 1),
  );
...
const renderFire = (
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    for (let column = 0; column < columns; column += 1) {
      ...
      const color =
        particleGlyph?.color ??
        (value < 0.025 ? "" : safePalette[paletteIndex]);
      const character =
        particleGlyph?.glyph ??
        (value < 0.025 ? " " : characters[characterIndex]);
