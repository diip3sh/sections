...
// Using component defaults. "use client" added on vendoring: the component is
// all hooks, pointer listeners and a WebGL context, none of which can run on
// the server.
  ...
  const tick = useCallback(
    (timestamp: number) => {
      ...
      if (state.held || state.peeling || state.sticking || holoDecayActive) {
        // Upstream self-scheduling rAF loop: `tick` refers to itself inside its
        // own useCallback. Safe at runtime — the reference is only read once a
        // frame fires, long after the binding is assigned — but it trips
        // react-hooks/immutability, and restructuring it would mean rewriting
