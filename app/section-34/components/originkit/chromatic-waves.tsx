// Chromatic Waves — Originkit
// Using component defaults, plus one added prop: `play`. Upstream hard-codes the
// RAF loop on, which leaves no way to honour `prefers-reduced-motion` or to stop
// the shader while the hero is scrolled off-screen. Every branch it needs was
// already here — `effectivePlay` gates the loop, the speed uniform and the
// static `renderOnce()` path — so the prop only supplies the value, and it
...
interface DottedBg2Props {
  ...
  paletteBias?: number;
  style?: CSSProperties;
  /** Drives the animation loop. `false` renders one static frame. */
  play?: boolean;
}
