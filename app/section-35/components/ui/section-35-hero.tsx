 ...
 * viewport only as far as 1141: past that the bands stand further apart than any
 ...
 * spreading those is what made an earlier full-height version read wrong. Above
 * 1141 the remainder collects as framed space under the steps, with the rails
 * running past it to the bottom edge.
 *
        ...
        */}
        <div className="contents desktop-sm:flex desktop-sm:max-h-[1141px] desktop-sm:flex-1 desktop-sm:flex-col">
          ...
          {/*
            ...
            is also the band that takes the surplus: `grow` against an `h-[453px]`
            basis opens it from 453 to whatever the capped stack leaves, while the
            nav and the steps keep the heights their content and padding give
            them. Below 1280 the band is fixed and the surplus stays under the
            steps, since those frames are taller than their viewports.
          */}
          <div
          ...
          >
            ...
            >
              ...
              which is what enlarges the figure. It is a box change rather than a
              `scale`, because the glyph count is fixed at 93: a bigger box spends
              those 93 columns over more pixels and redraws the art larger and
              still crisp, where a transform would resample the canvas and soften
              every glyph.

              Under `contain` only one axis can drive that. The fit takes
              `min(w/1024, h/1536)`, and at every width this design runs the box
              is the wider of the two, so the height is the whole zoom and any
              extra width is empty margin either side of the art — Figma's own
              653x580 plate is mostly that. `aspect-[1024/1536]` collapses the two
              into one number: the box *is* the art, so the width follows and
              there is no inert margin to reason about.

              105% of the band, bottom-anchored. The art then stands on the bottom
              rule with the whole figure in frame and overhangs the top by 5% —
              which is the empty margin above the head in the source (its crown
              starts at y60 of 1536, 3.9% down), so the crop takes the slack and
              not the head. Anchoring it the other way is what cut the figure off
              at the chest before. Being a ratio of the band, the framing holds
              from the 453 the design draws to the ~766 the band reaches under the
              1141 cap — that being 1141 less the 94px nav and the steps' own
              content height. At that height the art comes out 511 wide against a
              510 cell, so it fills the column exactly as the band opens.
            */}
              <div className="relative h-[128%] w-[128%] shrink-0 ipad:w-[573px] desktop-sm:aspect-[1024/1536] desktop-sm:translate-y-[10%] desktop-sm:h-[105%] desktop-sm:w-auto desktop-sm:self-end">
                <AsciiPortrait />
              </div>
            </div>
