 ...
 * not peel. Each `sticker-*.svg` in `public/section-44/` is therefore Figma's
...
type StickerProps = {
  /** Artwork slug — `public/section-44/sticker-<name>.svg`. */
  name: string;
  label: string;
...
export const Sticker = ({ name, label, size, className }: StickerProps) => {
  ...
  return (
    ...
    >
      {/*
        ...
        Figma's rest shadow scales with the card (0.587 at 62, 0.776 at 82 — the
        same 1.32), so it is derived from the box rather than restated per frame.
      ...
      */}
      {box !== null && (
        <StickerDrag
          key={box}
          image={{ src: `/section-44/sticker-${name}.svg`, alt: label }}
          imageWidth={box}
          imageHeight={box}
          tilt={45}
          lighting
          ...
          sheenMode="sheen"
          elevation={10}
          staticShadow={`${(box * 0.0074).toFixed(3)}px ${(box * 0.0074).toFixed(3)}px ${(box * 0.00736).toFixed(3)}px rgba(0, 0, 0, 0.24)`}
